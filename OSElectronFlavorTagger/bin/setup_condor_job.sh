...
jobDir=${srcDir}/OSElectronFlavorTagger/bin/flavortagOSElectron_job_${timeNow}

libFile=lib_${timeNow}.tar.gz
cfgFile=flavortagOSElectron_${timeNow}.cfg
scriptFile=flavortagOSElectron_${timeNow}.sh
jobConfigFile=flavortagOSElectron_${timeNow}.jobconfig
...
sed -e "s#JOBDIR#${jobDir}#g" -e "s#SCRIPTFILE#${scriptFile}#g" -e "s#EVENTSPERJOB#${eventsPerJob}#g" -e "s#LIBFILE#${libFile}#g" -e "s#EXEDIR#${exeDir}#g" -e "s#CFGFILE#${cfgFile}#g" -e "s#TREELISTFILE#${treeListFile}#g" -e "s#OUTFILES#${outFiles}#g" -e "s#NJOBS#${nJobs}#g" flavortagOSElectron_template.jobconfig > ${jobDir}/${jobConfigFile}

# Create shell script executable from template inside the job directory
echo Creating file ${jobDir}/${scriptFile}
sed -e "s#LIBFILE#${libFile}#g" -e "s#CFGFILE#${cfgFile}#g" -e "s#SAMPLENAME#${sampleName}#g" -e "s#EVTSELECTION#${evtSelection}#g" -e "s#SECONDNTUPLEBASENAME#${secondNtupleBaseName}#g" -e "s#TIMENOW#${timeNow}#g" flavortagOSElectron_template.sh > ${jobDir}/${scriptFile}
...
echo Copying executable ${exeDir}/flavortagOSElectron to job directory
cp ${exeDir}/flavortagOSElectron ${jobDir}/flavortagOSElectron
