...
{
  if(selection.compare("eleTagTightV2") == 0 || selection.compare("eleTagTightV1") == 0 || selection.compare("eleTagTightV0") == 0)
  {
...
{
  if(selection.compare("TightV1") == 0)
  {
    ...
    {
      int j = tkJPsi[i];
      if(trkPt->at(j) < 4.0) // For JPsiTrktrk
      {
        return false;
      }
  ...
  else if(selection.compare("TightV0") == 0) // This is the same as IsTightJPsi() in Alberto code.
  {
    if(fabs(svtMass->at(iJPsi) - constants::jPsiMass) > 0.15)
    {
      return false;
...
bool MGSelector::SelectPhi(const int iPhi, const std::string selection)
{
  if(selection.compare("TightV1") == 0)
  {
    ...
    {
      int j = tkPhi[i];
      if(trkPt->at(j) < 0.8) // For JpsiTrkTrk
      {
        return false;
      }
      if(fabs(trkEta->at(j)) > 2.5)
...
int MGSelector::SelectBestBsToJPsiPhi(const std::string selection, int& iBestPV)
{
  ...
  if(selection.compare("TightV2") == 0)
  {
    int index = -1;
    float best = 0.;
  ...
  else if(selection.compare("LooseV0") == 0) // This is the same as GetBestBstrange() in Alberto code.
  {
    int index = -1;
    float bestChi2 = 1e9;
    for(int iB = 0; iB < nSVertices; iB++)
